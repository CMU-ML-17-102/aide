...
/*
	USAGE:
	hist = SVRGLinReg(w, Xt, y, lambda, stepsize, iVals, m);
	==================================================================
	INPUT PARAMETERS:
...
/// SVRG_dense runs the SVRG algorithm for solving regularized 
/// linear regression on dense data provided
...
mxArray* SVRG_dense(int nlhs, const mxArray *prhs[]) {
  ...
  lossType = (long)mxGetScalar(prhs[7]); // Loss type
	if (nlhs == 1) {
		evalf = true;
	}
	...
	for (k = 0; k < iters; k++)
	{
		...
		compute_full_gradient(lossType, Xt, w, y, gold, n, d, lambda);
		// Save the point where full gradient was computed
		...
		for (i = 0; i < m[k]; i++) {
			...
			update_test_point_dense_S2GD(Xt + d*idx, w, wold, gold, 
				gradient, gradientold, d, stepSize, lambda);
		}
	}
...
/// S2GD_sparse runs the S2GD algorithm for solving regularized 
...
mxArray* SVRG_sparse(int nlhs, const mxArray *prhs[]) {
  ...
  lossType = (long)mxGetScalar(prhs[7]); // Loss type
	...
	for (k = 0; k < iters; k++)
	{
		// Evaluate function value if output requested
		if (evalf == true) {
			hist[k] = compute_function_value_sparse(lossType, w, Xt, y, n, d, lambda, ir, jc);
		}

		// Initially, compute full gradient at current point w.
		compute_full_gradient_sparse(lossType, Xt, w, y, gold, n, d, lambda, ir, jc);
		...
		for (i = 0; i < m[k]; i++) {
			...
			lazy_update_S2GD(w, wold, gold, last_seen, stepSize, 
                       lambda, i, ir, jc + idx);
      ...
      gradientold = compute_scalar_gradient_sparse(lossType, Xt + jc[idx], wold, 
              y[idx], jc[idx + 1] - jc[idx], ir + jc[idx]);
		...
		finish_lazy_updates_S2GD(w, wold, gold, last_seen, stepSize, 
                             lambda, m[k], d);
	}
