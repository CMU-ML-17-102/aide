...
/*
	USAGE:
	hist = SVRGLocal(w, Xt, y, lambda, grad, stepsize, gold, iVals, m, ...
                   lossType);
	==================================================================
	INPUT PARAMETERS:
...
/// SVRG_dense runs the SVRGLocal on dense data provided
  ...
  lossType = (long)mxGetScalar(prhs[8]); // Loss type
	...
	for (k = 0; k < iters; k++)
	{
		...
		for (i = 0; i < m[k]; i++) {
			...
			gradient = compute_scalar_gradient_sparse(lossType, Xt + jc[idx], w, y[idx], 
								jc[idx + 1] - jc[idx], ir + jc[idx]);
			gradientold = compute_scalar_gradient_sparse(lossType, Xt + jc[idx], wold, y[idx], 
								jc[idx + 1] - jc[idx], ir + jc[idx]);
	...
	if (evalf == true) {
		hist[iters] = compute_function_value_sparse(lossType, w, Xt, y, n, d, lambda, ir, jc);
	}
